...
struct bench_params {
    ...
    std::string moe_sere_policy = "miss";
...
int main(int argc, char ** argv) {
    ...
    if (!parse_args(argc, argv, p)) {
        fprintf(stderr, "Usage: llama-moe-bench --model <path> [--prompt-file PATH | -p TEXT] --pp N --tg N [--repeat N] [--moe-cache-vram-mb MB] [--moe-predictor lru|eamc] [--moe-eamc-path PATH] [--moe-sere-path PATH --moe-sere-top-k N --moe-sere-threshold F --moe-sere-policy paper|miss] [--moe-profile-csv PATH] [--moe-profile-summary PATH] [--moe-host-cache off|pageable|pinned] [--moe-host-cache-preload none|all] [--page-cache-policy natural|cold|hot] [--token-trace PATH] [--logits-bin PATH] [--moe-reset-cache-between-repeats] [--moe-warm-cache] [--moe-hot-start] [-ub N]\n");
        return 1;
    }
