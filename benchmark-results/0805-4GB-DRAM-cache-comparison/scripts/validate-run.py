...
def validate(args):
    ...
    if not residency or not prepare or not process_io:
        ...
        io_keys = ("model", "prefill", "decode") if args.mode == "control" else ("prefill", "decode")
        checked_reads = sum(int(process_io[key]) for key in io_keys)
        if checked_reads > args.model_size // 100:
            interval = "post-prepare" if args.mode == "control" else "timed inference"
    ...
    expected_host = "off" if args.mode == "control" else "pinned"
    expected_preload = "hotset" if args.mode == "host4gb-hot" else "none"
    ...
    if args.mode != "control":
        initial = named(
            summary,
            r"^Host cache initial: blobs=(?P<blobs>\d+) experts=(?P<experts>\d+) bytes=(?P<gib>[0-9.]+) GB$",
        )
        management = named(
            summary,
            r"^Host cache management: admissions=(?P<admissions>\d+) evictions=(?P<evictions>\d+) "
            r"bypasses=(?P<bypasses>\d+) active_leases=(?P<leases>\d+)$",
        )
        ready = named(
            summary,
            r"^Host cache ready: blobs=(?P<ready>\d+)/(?P<total>\d+) experts=(?P<experts>\d+) "
            r"slots_per_layer=(?P<slots>\d+)",
        )
        expected_initial = (0, 0) if args.mode == "host4gb-cold" else (6720, 2240)
        if not initial or (int(initial["blobs"]), int(initial["experts"])) != expected_initial:
            errors.append(f"initial Host state invalid: expected={expected_initial} actual={initial}")
        if not ready or int(ready["total"]) != 6720 or int(ready["slots"]) != 56:
            errors.append(f"bounded Host shape invalid: {ready}")
        if management is None or int(management["leases"]) != 0:
            errors.append(f"Host leases not fully released: {management}")
        ...
        if not verified or int(verified["failures"]) != 0:
            errors.append(f"Host verification invalid: {verified}")
