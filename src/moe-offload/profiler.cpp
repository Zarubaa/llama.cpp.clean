...
void profiler::record_request(const profile_request_row & row) {
    // Keep profiling classification aligned with SERE eligibility: only an
    // explicitly marked decode request belongs to decode. Unknown one-token
    // requests are conservatively handled as prefill by the slot callback.
    profile_phase_stats & stats = row.phase == "decode" ? decode_stats : prefill_stats;
    ++stats.requests;
...
std::string format_summary(
    ...
    const double vram_device_baseline_gib = bytes_to_gib(ctx.vram_device_baseline_bytes);
    const double vram_device_peak_gib = bytes_to_gib(ctx.vram_device_peak_bytes);
    ...
    if (ctx.sere_top_k > 0) {
        out << "SERE: top-k=" << ctx.sere_top_k
            ...
            << "  mode=" << (ctx.sere_shadow ? "shadow" : "active")
    ...
    if (ctx.n_ubatch > 0) {
        ...
        if (ctx.n_slots > 0 || ctx.n_active_slots > 0 || ctx.n_experts > 0) {
            out << "  slots=" << ctx.n_slots << "/" << ctx.n_active_slots << "/" << ctx.n_experts
    ...
    if (ctx.vram_peak_bytes > 0 || ctx.vram_total_bytes > 0 ||
            ctx.vram_device_baseline_bytes > 0 || ctx.vram_device_peak_bytes > 0) {
        ...
        if (ctx.vram_device_baseline_bytes > 0 || ctx.vram_device_peak_bytes > 0) {
            out << "VRAM device used: baseline=" << vram_device_baseline_gib
                << " GB, peak=" << vram_device_peak_gib << " GB"
                << "  (whole visible GPU(s), includes other processes)\n";
        }
    } else {
