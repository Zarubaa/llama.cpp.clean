...
// ── I/O worker (source staging + asynchronous H2D) ─────────────────────
            ...
            while (queue.pop(req)) {
                ...
                if (req.h2d && req.gpu_dst) {
                    ...
                    if (io_h2d_async_timed(req.gpu_dst, req.pinned_buf, req.blob_size,
                        ...
                        if (req.prefill_stream &&
                                !io_prefill_stream_register_ready(req.slot, req.kind, ev_end)) {
                            req.ok = false;
                            req.io_error = EINVAL;
                        }
                    } else {
                        ...
                        if (req.prefill_stream) {
                            io_prefill_stream_register_failed(req.slot, req.kind);
                        }
                    }
                }
...
void   io_prefill_stream_register_failed(int32_t slot, int kind) {
    moe_io_cuda_prefill_stream_register_failed(slot, kind);
}
