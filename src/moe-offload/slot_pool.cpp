...
struct slot_pool_state {
    ...
    // Persistent per-layer cache slots that survive across callbacks and are
    // managed by the LRU/EAMC predictor.
    uint32_t n_slots = 0;
    // Per-layer capacity visible to a single MoE callback:
    // persistent slots for that layer + transient shared-scratch slots.
    uint32_t n_active_slots = 0;
    // Shared scratch slots appended once after all per-layer persistent
    // segments in the global combined tensor axis.
    uint32_t n_scratch_slots = 0;
    // Physical expert axis of the global combined tensor:
    // [L0 persistent][L1 persistent]...[Ln persistent][shared scratch].
    uint32_t n_global_slots = 0;
    // Q4_K_M can mix quantization types for the same expert kind across layers
    // (for example q5_K and q6_K down experts). GGML tensors have a single
    // dtype, so keep one combined axis per (expert kind, dtype) while preserving
    // the same numeric slot-id layout across all of them.
    std::unordered_map<int, ggml_tensor *> global_slot_tensors;
    ...
    struct layer_cache {
        std::vector<int32_t> slot_to_expert;          // persistent slots only, -1 = free
    ...
    sere_policy sere_mode = sere_policy::paper;
...
uint32_t compute_active_slots(uint32_t persistent_slots, const manifest & mf) {
    if (mf.n_experts_per_layer == 0 || persistent_slots == 0) {
        return persistent_slots;
    }

    uint32_t scratch = mf.n_experts_per_layer > persistent_slots ?
        mf.n_experts_per_layer - persistent_slots : 0;

    if (std::getenv("LLAMA_MOE_DISABLE_SHARED_SCRATCH")) {
        scratch = 0;
    } else if (const char * env = std::getenv("LLAMA_MOE_SHARED_SCRATCH_SLOTS")) {
        char * end = nullptr;
        const unsigned long parsed = std::strtoul(env, &end, 10);
        if (end != env) {
            scratch = (uint32_t) parsed;
        }
    }

    const uint32_t max_scratch = mf.n_experts_per_layer - persistent_slots;
    if (scratch > max_scratch) {
        scratch = max_scratch;
    }
    return persistent_slots + scratch;
}
...
uint32_t compute_global_slots(uint32_t persistent_slots, uint32_t scratch_slots, const manifest & mf) {
    if (mf.n_layers == 0) {
        return persistent_slots + scratch_slots;
    }
    if (persistent_slots < mf.n_experts_per_layer) {
        return persistent_slots * mf.n_layers + scratch_slots;
    }
    return persistent_slots;
}

uint32_t persistent_global_slot(const slot_pool_state & s, uint32_t logical_layer, uint32_t local_slot) {
    if (s.n_slots < get_manifest().n_experts_per_layer) {
        return logical_layer * s.n_slots + local_slot;
    }
    return local_slot;
}

uint32_t scratch_global_slot_begin(const slot_pool_state & s, const manifest & mf) {
    if (s.n_slots < mf.n_experts_per_layer) {
        return s.n_slots * mf.n_layers;
    }
    return s.n_slots;
}

int global_slot_tensor_key(expert_kind kind, ggml_type type) {
    return ((int) kind << 16) | ((int) type & 0xffff);
}
...
bool configure_slot_pool() {
    ...
    s.n_active_slots = compute_active_slots(s.n_slots, mf);
    s.n_scratch_slots = scratch_slots_from_active(s.n_slots, s.n_active_slots);
    s.n_global_slots = compute_global_slots(s.n_slots, s.n_scratch_slots, mf);
    s.global_slot_tensors.clear();
    ...
    if (opts.sere_top_k > 0) {
        ...
        if (!make_sere_model_binding(mf, binding, error)) {
            LLAMA_LOG_ERROR("%s: cannot bind SERE sidecar to model: %s\n", __func__, error.c_str());
            return false;
        }
        if (!s.sere_similarity.load(opts.sere_path, binding, error)) {
            LLAMA_LOG_ERROR("%s: %s\n", __func__, error.c_str());
            return false;
        }
    }
    ...
    if (opts.sere_top_k > 0) {
        LLAMA_LOG_INFO("%s: SERE decode rerouting enabled: top-k=%d threshold=%.4f policy=%s mode=%s metric=%u\n",
                __func__, opts.sere_top_k, opts.sere_threshold,
                sere_policy_name(s.sere_mode), opts.sere_shadow ? "shadow" : "active",
                s.sere_similarity.metric());
    }
    return true;
}
...
uint32_t active_slots_per_layer() {
    auto & s = state();
    std::lock_guard<std::mutex> lock(s.mutex);
    return s.n_active_slots;
}
...
uint32_t recommended_ubatch(uint32_t requested, uint32_t n_expert_used, float safety) {
    const uint32_t active_slots = active_slots_per_layer();
    const uint32_t n_experts = n_experts_per_layer();
    if (active_slots == 0 || n_expert_used == 0 || n_experts == 0) {
        return requested;
    }

    // The streaming callback gates on the actual deduplicated expert IDs used
    // by the current layer. Once the active axis covers every expert in a
    // layer, extra tokens cannot require more active slots than n_experts, so
    // there is no MoE-specific reason to cap ubatch by active_slots / top_k.
    if (active_slots >= n_experts) {
        return requested;
    }
    ...
    const double raw = std::floor((double) active_slots * (double) safety / (double) n_expert_used);
...
ggml_tensor * intercept_expert_tensor(
    ...
    const int global_key = global_slot_tensor_key(kind, src_meta->type);

    uint32_t n_slots = 0;
    uint32_t n_active_slots = 0;
    uint32_t n_global_slots = 0;
    bool use_global_axis = false;
    ggml_tensor * existing_global_slot = nullptr;
    {
        auto & s = state();
        std::lock_guard<std::mutex> lock(s.mutex);
        n_slots = s.n_slots;
        n_active_slots = s.n_active_slots;
        n_global_slots = s.n_global_slots;
        use_global_axis = s.configured && s.n_slots < get_manifest().n_experts_per_layer;
        if (use_global_axis) {
            auto it = s.global_slot_tensors.find(global_key);
            if (it != s.global_slot_tensors.end()) {
                existing_global_slot = it->second;
            }
        }
    }
    if (n_slots == 0 || n_active_slots == 0 || n_global_slots == 0) {
        return nullptr;
    }
    ...
    const int64_t n_slot_axis = use_global_axis ? (int64_t) n_global_slots :
        (full_axis_guard ? n_expert : (int64_t) n_active_slots);
    if (full_axis_guard && !use_global_axis) {
    ...
    std::snprintf(global_name, sizeof(global_name), "moe.global_slot.%s.%s",
            expert_kind_name(kind), ggml_type_name(src_meta->type));
    const std::string slot_name = use_global_axis ? std::string(global_name) : src_name + ".slot";

    ggml_tensor * slot = existing_global_slot;
    if (slot) {
        if (slot->type != src_meta->type || slot->ne[0] != d_in || slot->ne[1] != d_out || slot->ne[2] != n_slot_axis) {
            std::ostringstream oss;
            oss << "intercept_expert_tensor: incompatible MoE expert shape for global combined tensor"
                << " tensor=" << src_name
                << " kind=" << expert_kind_name(kind)
                << " existing_type=" << ggml_type_name(slot->type)
                << " incoming_type=" << ggml_type_name(src_meta->type)
                << " existing_ne=[" << slot->ne[0] << "," << slot->ne[1] << "," << slot->ne[2] << "]"
                << " incoming_ne=[" << d_in << "," << d_out << "," << n_slot_axis << "]"
                << " src_ne=[" << src_meta->ne[0] << "," << src_meta->ne[1] << "," << src_meta->ne[2] << "]";
            throw std::runtime_error(oss.str());
        }
    } else {
        slot = ml.create_unfiled_tensor(
    ...
    auto & s = state();
    std::lock_guard<std::mutex> lock(s.mutex);
    if (use_global_axis && s.global_slot_tensors.find(global_key) == s.global_slot_tensors.end()) {
        s.global_slot_tensors[global_key] = slot;
    }
    ...
    if (!logged_first) {
        LLAMA_LOG_INFO("%s: intercepted %s -> %s [%lld, %lld, axis=%lld, active/layer=%u, persistent/layer=%u] dtype=%s\n",
                __func__, src_name.c_str(), slot_name.c_str(),
                (long long) d_in, (long long) d_out, (long long) n_slot_axis,
                n_active_slots, n_slots, ggml_type_name(src_meta->type));
        logged_first = true;
    }
    return slot;
}
...
bool prefetch_all_experts() {
    ...
    // produce valid all-zero Q4_K blocks which the kernel handles safely.
    {
        ...
        std::unordered_set<ggml_tensor *> zeroed;
...
bool slot_pool_hot_start(const std::vector<std::vector<int>> & experts_by_layer) {
    ...
    for (uint32_t logical = 0; logical < mf.n_layers && logical < experts_by_layer.size(); ++logical) {
        ...
        for (int expert : experts_by_layer[(size_t) logical]) {
            ...
            const uint32_t global_slot = persistent_global_slot(s, logical, slot);
            if (!load_expert_into_slot(s, mf, logical, expert, (int32_t) global_slot)) {
                return false;
            }
...
bool moe_eval_callback(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    if (diag_slot_start && logical < 3) {
        ...
        if (st && st->buffer) {
            ...
            for (int si = 0; si < 5; ++si) {
                ...
                const uint32_t global_slot = persistent_global_slot(s, (uint32_t) logical, (uint32_t) s_idx);
                size_t off = (size_t) global_slot * stride;
                ...
                fprintf(stderr, "[slot-hash-start] tok=%llu L%d slot=%d global_slot=%u hash=0x%016llx%s\n",
                        (unsigned long long) s.token_idx, logical, s_idx, global_slot,
    ...
    if (is_decode_request && n_tokens == 1 &&
        ...
        rerouted_ids = sere_reroute_decode(
                ...
                resident,
                sere_stats);
        // Shadow mode evaluates the same counterfactual mapping against the
        // current raw-route cache state, but deliberately leaves every stateful
        // and numerical path below on the router's original expert IDs.
        if (!runtime_opts.sere_shadow) {
    ...
    // Architectural constraint for the combined-axis path: all unique experts
    // selected this batch must fit in the physical expert axis consumed by
    // mul_mat_id. Persistent cache slots hold long-lived experts; the extra
    // active slots are transient shared scratch for this callback's misses.
    if (uniq.size() > s.n_active_slots) {
        static int err_logged = 0;
        if (err_logged < 3) {
            LLAMA_LOG_ERROR("moe_eval_callback: n_uniq=%zu exceeds active_slots=%u "
                    "(layer=%d, n_tokens*n_expert_used=%lld). "
                    "Increase LLAMA_MOE_SHARED_SCRATCH_SLOTS, disable scratch limiting, "
                    "or reduce batch size.\n",
                    uniq.size(), s.n_active_slots, logical, (long long) n_elem);
            ++err_logged;
        }
        GGML_ABORT("MoE-offload: per-batch unique experts exceeds active slot axis");
    }
    ...
    if (debug_slot_fingerprint()) {
        ...
        if (fp_tensor && fp_tensor->buffer) {
            ...
            for (int32_t e : uniq) {
                ...
                int32_t sl = (int32_t) persistent_global_slot(s, (uint32_t) logical, (uint32_t) it->second);
    ...
    for (int32_t e : uniq) {
        ...
        if (slot < 0) {
            LLAMA_LOG_ERROR("moe_eval_callback: no evictable victim at L%d "
                    ...
                    "Increase --moe-cache-vram-mb or LLAMA_MOE_SHARED_SCRATCH_SLOTS.\n",
        ...
        for (int k = 0; k < EXPERT_KIND_COUNT; ++k) {
            ...
            const size_t buf_size = ggml_backend_buffer_get_size(slot_tensor->buffer);
            const size_t write_off = (size_t) write_slot * stride;
            ...
            if (tensor_off + write_off + rec.size > buf_size) {
                LLAMA_LOG_ERROR("moe_eval_callback: OOB write tensor_off=%zu slot_off=%zu rec=%llu buf_size=%zu (L%d e%d k%d slot=%d)\n",
                        tensor_off, write_off, (unsigned long long) rec.size, buf_size, logical, e, k, (int) write_slot);
        ...
        // callback does not pick them as victims. Scratch slots are transient
        // and live only in call_exp2slot for this graph segment.
        call_exp2slot[e] = write_slot;
        if (persistent_slot) {
    ...
    if (std::getenv("LLAMA_MOE_DEBUG_EVICT")) {
        fprintf(stderr, "[evict] tok=%llu L%d uniq=%zu hits=%d misses=%d free_alloc=%d scratch_alloc=%d evictions=%d persistent_slots=%u active_slots=%u\n",
                (unsigned long long) row_token_idx, logical, uniq.size(),
                dbg_hits, dbg_misses, dbg_free_alloc, dbg_scratch_alloc,
    ...
    if (diag_slot_hash && logical < 3) {
        ...
        if (st_for_read && st_for_read->buffer) {
            ...
            for (int si = 0; si < kMonCount; ++si) {
                ...
                fprintf(stderr, "[slot-hash-end] tok=%llu L%d slot=%d global_slot=%u hash=0x%016llx%s\n",
                        (unsigned long long) row_token_idx, logical, s_idx, global_slot,
