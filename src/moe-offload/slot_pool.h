...
// uniform persistent cache slot count per MoE layer using the configured cache
...
// Benchmark-only hot start: preload selected experts into persistent slots
...
// Returns the active expert axis consumed by ggml_mul_mat_id:
// persistent slots + transient shared-scratch slots.
LLAMA_API uint32_t active_slots_per_layer();

// Returns the routed expert count recorded in the MoE manifest.
LLAMA_API uint32_t n_experts_per_layer();

// Recommended effective ubatch for streaming mode. If the active expert axis
// covers all experts in a layer, the requested ubatch is preserved because the
// deduplicated per-layer miss set cannot exceed n_experts. Otherwise the
// recommendation is derived from the current active slot axis and model top-k.
// `safety` defaults to a worst-case unique-expert bound when set to 1.0; lower
// values are more conservative and values above 1.0 are diagnostic/optimistic.
...
//   - Allocates a slot tensor [d_in, d_out, active_slots] on the layer's preferred
